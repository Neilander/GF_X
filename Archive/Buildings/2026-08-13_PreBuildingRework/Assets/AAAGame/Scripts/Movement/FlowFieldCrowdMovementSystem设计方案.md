...
v72 完成 Flow 配置入口的 fixed authority 收口。`DistanceConversionRate` 和四档 agent type 碰撞半径不再经过 `GF.Config.GetFloat`；配置原始 invariant decimal 文本直接参与“表值 Fix64 raw × 十进制倍率”，只在最终 world raw 向外量化一次。红测 `0.015136718751` 直接得到 raw 63，而旧 float 中转仅得到 raw 62。world、world build job、runtime-dirty job 和 authored navigation source 均持有 `AgentRadiusFixedRaw`；float 半径只由 fixed 单向派生为 Unity/诊断 shadow，不能参与落格、净空、成本、碰撞或 authority digest。

结构门禁现同时检查 Flow、`DistanceUnitConverter`、`CharacterDataDetailAccessor`、Buff、Card、Defend 和 Reward，禁止重新引入 `GF.Config.GetFloat`。生产 fixed 配置缺失、非法、非正或溢出会明确报错，没有 fallback。`SuperLargeUnitCollisionRadius` 已纳入 agent type resolver；Defend 的 world speed 反向换算使用同一 decimal 边界，避免先把倍率量化后再除法产生额外 raw 误差。
