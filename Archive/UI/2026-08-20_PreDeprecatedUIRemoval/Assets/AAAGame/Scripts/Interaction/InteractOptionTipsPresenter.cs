...
public class InteractOptionTipsPresenter : MonoBehaviour
{
    ...
    private static bool ShouldShowBuildTips(InteractionHost target)
    {
        ...
        if (LogicWallRuntime.IsWallBuilding(building.buildingData)
            && LogicWallRuntime.TryGetPreviewCell(building.LogicEntityId, out WallGridCell wallCell)
            && LogicWallRuntime.GetAdjacentBranches(wallCell, building.OwnerFactionID).Count > 0)
            return false;
    ...
    private static bool ShouldShowUpgradeTips(InteractionHost target)
    {
        ...
        if (building != null
            && LogicWallRuntime.IsWallBuilding(building.buildingData)
