...
public sealed class LogicMovementRegionConstraintServiceTests
{
    ...
    [Test]
    public void TutorialBoundary_AllowsTargetExitAndFriendlyFollowButBlocksOtherEnemyStronghold()
    {
        LogicStrongholdMap.Clear();
        LogicStrongholdMap.Initialize(
            FixVector2.Zero,
            ...
            new[]
            {
                ...
                new LogicStrongholdCellDefinition("target", 1, 0, EntitySideHelper.EnemyFactionId),
                new LogicStrongholdCellDefinition("other", 2, 0, EntitySideHelper.EnemyFactionId),
            });
        SimEntityContext player = CreateEntity(SideType.PlayerSide);
        LogicPhaseCommandService.ScheduleForNextFrame(GamePhase.Invade);
        LogicTimeControlService.BeginFrame(1);
        ...
        LogicMovementRegionConstraintService.SetTutorialStrongholdBoundary("target");
        var after = new LogicStateHasher();
        LogicMovementRegionConstraintService.WriteDeterministicState(after);

        FixVector2 target = StrongholdPoint(1);
        FixVector2 outside = StrongholdPoint(-1);
        Assert.AreEqual(
            outside,
            LogicMovementRegionConstraintService.ResolvePosition(
                player,
                target,
                outside,
                out LogicMovementRegionConstraintFailure exitFailure));
        Assert.AreEqual(LogicMovementRegionConstraintFailure.None, exitFailure);
        ...
        Assert.AreEqual(
            ...
            LogicMovementRegionConstraintService.ResolvePosition(
                ...
                outside,
                target,
                out LogicMovementRegionConstraintFailure followFailure));
        Assert.AreEqual(LogicMovementRegionConstraintFailure.None, followFailure);
        ...
        Assert.AreEqual(outside, resolved);
