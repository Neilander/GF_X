...
public class PlayerHandManager : MonoBehaviour
{
    ...
    private void InitializeHand()
    {
        ...
        GF.Log($"初始化手牌：{handCards.Count} 张");
    }
    ...
    public void DrawRandomCards(int count)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            {
                GF.LogWarning($"手牌已满！最多 {maxHandSize} 张");
                break;
            }
            ...
            if (drawnCard != null)
            {
                ...
                GF.Log($"抽取卡牌：{drawnCard.cardName}（权重：{drawnCard.dropWeight}）");
            }
        }
    ...
    public bool AddCard(CardData card)
    {
        ...
        handCards.Add(card);
        OnHandChanged?.Invoke(handCards);
        GF.Log($"添加卡牌：{card.cardName}，当前手牌：{handCards.Count}/{maxHandSize}");
        return true;
    }
    ...
    public bool RemoveCard(CardData card)
    {
        ...
        OnHandChanged?.Invoke(handCards);
        GF.Log($"移除卡牌：{card.cardName}，当前手牌：{handCards.Count}/{maxHandSize}");
        return true;
    }
    ...
    public bool RemoveCardAt(int index)
    {
        ...
        OnHandChanged?.Invoke(handCards);
        GF.Log($"移除卡牌：{card.cardName}（索引{index}），当前手牌：{handCards.Count}/{maxHandSize}");
        return true;
    }
