...
/// UniqueValues: 1,1
/// </summary>
public class TechBuilSouvenirStandLv2EffectSO : TechEffectSO
{
    public override void Activate(TechEffectContext context)
    {
        ...
        {
            Debug.LogWarning($"[{nameof(TechBuilSouvenirStandLv2EffectSO)}] ResolvedScope 无 BuildingInstanceIds, techId={context.TechId}");
            return;
        }
    ...
    public override BuffData CreateBuildingScopedBuff(TechData techData, string techId)
    {
        ...
        Debug.LogWarning($"[{nameof(TechBuilSouvenirStandLv2EffectSO)}] 未实现字段: 日产出+<val1>（建筑属性） (techId={techId})");
