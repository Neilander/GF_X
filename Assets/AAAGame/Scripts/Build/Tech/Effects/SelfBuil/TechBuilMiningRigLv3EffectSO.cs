...
/// <summary>
/// TechID: Tech_Buil_MiningRig_Lv3
/// ScopeType: SelfBuil
/// 策划描述: 日产出+<val1>
/// UniqueValues: 2
/// </summary>
public class TechBuilMiningRigLv3EffectSO : TechEffectSO
{
    public override void Activate(TechEffectContext context)
    {
        ...
        {
            Debug.LogWarning($"[{nameof(TechBuilMiningRigLv3EffectSO)}] ResolvedScope 无 BuildingInstanceIds, techId={context.TechId}");
            return;
        }
    ...
    public override BuffData CreateBuildingScopedBuff(TechData techData, string techId)
    {
        ...
        Debug.LogWarning($"[{nameof(TechBuilMiningRigLv3EffectSO)}] 未实现字段: 日产出+<val1>（建筑属性） (techId={techId})");
