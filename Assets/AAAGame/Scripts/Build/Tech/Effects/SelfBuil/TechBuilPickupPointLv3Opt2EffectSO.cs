...
/// UniqueValues: 2,60
/// </summary>
public class TechBuilPickupPointLv3Opt2EffectSO : TechEffectSO
{
    public override void Activate(TechEffectContext context)
    {
        ...
        {
            Debug.LogWarning($"[{nameof(TechBuilPickupPointLv3Opt2EffectSO)}] ResolvedScope 无 BuildingInstanceIds, techId={context.TechId}");
            return;
        }
    ...
    public override BuffData CreateBuildingScopedBuff(TechData techData, string techId)
    {
        ...
        Debug.LogWarning($"[{nameof(TechBuilPickupPointLv3Opt2EffectSO)}] 未实现字段: 兵力-<val1>（建筑属性） (techId={techId})");
