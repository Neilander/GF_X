...
/// UniqueValues: 2
/// </summary>
public class TechBuilParcelLockerLv3EffectSO : TechEffectSO
{
    public override void Activate(TechEffectContext context)
    {
        ...
        {
            Debug.LogWarning($"[{nameof(TechBuilParcelLockerLv3EffectSO)}] ResolvedScope 无 BuildingInstanceIds, techId={context.TechId}");
            return;
        }
    ...
    public override BuffData CreateBuildingScopedBuff(TechData techData, string techId)
    {
        ...
        Debug.LogWarning($"[{nameof(TechBuilParcelLockerLv3EffectSO)}] 未实现字段: 上限+<val1>（建筑属性） (techId={techId})");
