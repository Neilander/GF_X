...
/// UniqueValues: 3
/// </summary>
public class TechBuilOtakuDeskLv2Opt1EffectSO : TechEffectSO
{
    public override void Activate(TechEffectContext context)
    {
        ...
        {
            Debug.LogWarning($"[{nameof(TechBuilOtakuDeskLv2Opt1EffectSO)}] ResolvedScope 无 BuildingInstanceIds, techId={context.TechId}");
            return;
        }
    ...
    public override BuffData CreateBuildingScopedBuff(TechData techData, string techId)
    {
        ...
        Debug.LogWarning($"[{nameof(TechBuilOtakuDeskLv2Opt1EffectSO)}] 未实现字段: 兵力+<val1>（建筑属性） (techId={techId})");
