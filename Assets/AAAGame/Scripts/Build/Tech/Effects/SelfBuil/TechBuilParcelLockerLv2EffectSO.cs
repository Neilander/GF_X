...
/// <summary>
/// TechID: Tech_Buil_ParcelLocker_Lv2
/// ScopeType: SelfBuil
/// 策划描述: 上限+<val1>
/// UniqueValues: 2
/// </summary>
public class TechBuilParcelLockerLv2EffectSO : TechEffectSO
{
    public override void Activate(TechEffectContext context)
    {
        ...
        {
            Debug.LogWarning($"[{nameof(TechBuilParcelLockerLv2EffectSO)}] ResolvedScope 无 BuildingInstanceIds, techId={context.TechId}");
            return;
        }
    ...
    public override BuffData CreateBuildingScopedBuff(TechData techData, string techId)
    {
        ...
        // TODO: 未实现字段 -> 上限+<val1>（建筑属性）
        Debug.LogWarning($"[{nameof(TechBuilParcelLockerLv2EffectSO)}] 未实现字段: 上限+<val1>（建筑属性） (techId={techId})");
