    ...
    public partial class CardUIForm : UIFormBase
    {
        ...
        protected override void OnInit(object userData)
        {
            ...
            if (trashBin != null)
            {
                ...
                Log.Info("[CardUI] TrashBin initialized successfully.");
            }
            else
            {
                Log.Warning("[CardUI] TrashBin is not configured in CardUIForm prefab. Discard feature will not work.");
            }
        ...
        private void RemoveHandCardItem(CardModel cardModel)
        {
            ...
            Log.Info($"[CardUI] Current hand card items count: {m_HandCardItemObjects.Count}");
            ...
            if (handCardArea == null)
            {
                if (handCardContainer != null && handCardContainer.parent != null)
                {
                    ...
                    if (parentRect != null)
                    {
                        ...
                        Log.Info($"[CardUI] IsOverHandCardArea (using parent): {result}");
                        return result;
                    }
                }
                Log.Info("[CardUI] IsOverHandCardArea: handCardArea is null, returning false");
                return false;
            }
            
            bool isOver = RectTransformUtility.RectangleContainsScreenPoint(
                handCardArea, screenPosition, GFBuiltin.UICamera);
            Log.Info($"[CardUI] IsOverHandCardArea: {isOver}");
            return isOver;
        }
        ...
        private bool IsInTrashBin(Vector2 screenPosition)
        {
            ...
            // 方法 1：使用 RectTransformUtility.RectangleContainsScreenPoint
            // 注意：这个方法在某些 Canvas 设置下可能不准确
            ...
            for (int i = 0; i < 4; i++)
            {
                // 使用 Camera.WorldToScreenPoint 而不是 RectTransformUtility
                if (uiCamera != null)
                {
            ...
            Log.Info($"[CardUI] TrashBin bounds - X:[{minX}, {maxX}], Y:[{minY}, {maxY}]");
            Log.Info($"[CardUI] Mouse position - X:{screenPosition.x}, Y:{screenPosition.y}");
            ...
            if (result2)
            {
                Log.Info("[CardUI] ✅ Card is over trash bin!");
            }
            else
            {
                Log.Warning("[CardUI] ❌ Card is NOT over trash bin!");
            }
