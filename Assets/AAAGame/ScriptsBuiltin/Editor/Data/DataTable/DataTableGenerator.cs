    ...
    public sealed class DataTableGenerator
    {
        ...
        private static string GenerateDataTableProperties(DataTableProcessor dataTableProcessor)
        {
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                {
                    string enumTypeName = GetEnumTypeName(dataTableProcessor, i);
                    if (dataTypeKeyword == "enum[]") enumTypeName += "[]";
                    else if (dataTypeKeyword == "enum?") enumTypeName += "?";

                    stringBuilder
                    .AppendLine("        /// <summary>")
                    .AppendFormat("        /// {0}", dataTableProcessor.GetComment(i)).AppendLine()
                    .AppendLine("        /// </summary>")
                    .AppendFormat("        public {0} {1}", enumTypeName, dataTableProcessor.GetName(i)).AppendLine()
                    .AppendLine("        {")
                    .AppendLine("            get;")
            ...
            for (int i = 0; i < dataTableProcessor.RawColumnCount; i++)
            {
                ...
                else if (dataTableProcessor.IsSystem(i))
                {
                    if (isArrayType > 0)
                    {
                        if (isArrayType == 1)
                        {
                            string elementTypeName = languageKeyword == "enum[]"
                                ? GetEnumTypeName(dataTableProcessor, i)
                                : languageKeyword.Replace("[]", string.Empty);
                            stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseArray<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i), elementTypeName).AppendLine();
                        }
                    ...
                    else
                    {
                        ...
                        else if (languageKeyword == "enum" || languageKeyword == "enum?")
                        {
                            ...
                            {
                                stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseNullableEnum<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i), enumTypeName).AppendLine();
                            }
                            else
                            {
                                stringBuilder.AppendFormat("            {0} = DataTableExtension.ParseEnum<{1}>(columnStrings[index++]);", dataTableProcessor.GetName(i), enumTypeName).AppendLine();
                            }
                        }
                        else
                        {
                ...
                else if (dataTableProcessor.IsSystem(i))
                {
                    if (isArrayType > 0)
                    {
                        if (isArrayType == 1)
                        {
                            string elementTypeName = languageKeyword == "enum[]"
                                ? GetEnumTypeName(dataTableProcessor, i)
                                : languageKeyword.Replace("[]", string.Empty);
                            stringBuilder.AppendFormat("                    {0} = binaryReader.ReadArray<{1}>();", dataTableProcessor.GetName(i), elementTypeName).AppendLine();
                        }
                        else if (isArrayType == 2)
                        {
                    ...
                    else
                    {
                        ...
                        else if (languageKeyword == "enum" || languageKeyword == "enum?")
                        {
                            ...
                            if (languageKeyword == "enum?")
                            {
                                stringBuilder.AppendFormat("                    {0} = binaryReader.ReadNullableEnum<{1}>();", dataTableProcessor.GetName(i), enumTypeName).AppendLine();
                            }
                            else
                            {
                                stringBuilder.AppendFormat("                    {0} = binaryReader.ReadEnum<{1}>();", dataTableProcessor.GetName(i), enumTypeName).AppendLine();
                            }
                        }
                        else
                        {
        ...
        private static string GetFirstEnumValue(DataTableProcessor dataTableProcessor, int col)
        {
            for (int i = dataTableProcessor.ContentStartRow; i < dataTableProcessor.RawRowCount; i++)
            {
                ...
                {
                    string value = dataTableProcessor.GetValue(i, col);
        ...
        private static string GetEnumTypeName(DataTableProcessor dataTableProcessor, int column)
        {
            string firstEnumValue = GetFirstEnumValue(dataTableProcessor, column);
            if (!DataTableExtension.TryParseEnum(firstEnumValue, out Type enumType))
            {
                throw new GameFrameworkException(Utility.Text.Format(
                    "Can not infer enum type for column '{0}' from value '{1}'. Expected format: EnumType.Item.",
                    dataTableProcessor.GetName(column),
                    firstEnumValue));
            }
