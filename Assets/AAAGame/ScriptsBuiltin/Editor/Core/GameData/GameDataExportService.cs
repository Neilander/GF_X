...
namespace UGF.EditorTools
{
    internal static class GameDataExportService
    {
        ...
        private static string[] ExcelSheetToTextLines(ExcelWorksheet excelSheet, bool normalizeCustomJsonColumns)
        {
            ...
            Dictionary<int, string> customJsonColumns = normalizeCustomJsonColumns ? GetCustomJsonColumns(excelSheet, effectiveEndColumn) : null;
            HashSet<int> dateTimeColumns = GetDateTimeColumns(excelSheet, effectiveEndColumn);
        ...
        {
            int startColumn = excelSheet.Dimension.Start.Column;
            int endColumn = excelSheet.Dimension.End.Column;
            int headerEndRow = Math.Min(excelSheet.Dimension.End.Row, DataTableContentStartRow - 1);
            ...
            for (int columnIndex = startColumn; columnIndex <= endColumn; columnIndex++)
            {
                ...
                {
                    object cellValue = excelSheet.GetValue(rowIndex, columnIndex);
        ...
        private static Dictionary<int, string> GetCustomJsonColumns(ExcelWorksheet excelSheet, int effectiveEndColumn)
        {
        ...
        private static HashSet<int> GetDateTimeColumns(ExcelWorksheet excelSheet, int effectiveEndColumn)
        {
            if (excelSheet.Dimension.End.Row < DataTableTypeRow)
            {
                return null;
