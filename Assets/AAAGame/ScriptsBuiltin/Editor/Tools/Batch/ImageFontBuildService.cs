...
namespace UGF.EditorTools
{
    ...
    {
        private const string TmpBitmapShaderPath = "Assets/AAAGame/Font/TextMesh Pro/Shaders/TMP_Bitmap-Custom-Atlas.shader";
        ...
        public static Font BuildLegacyFont(CharacterInfo[] characterInfos, Texture2D texture, string outputFontPath)
        {
            ...
            Shader fontShader = LoadLegacyFontShader();
            var material = GetOrCreateAsset(outputMaterialPath, () => new Material(fontShader), "普通艺术字材质");
            if (material == null)
            {
                return null;
            }
        ...
        {
            Shader bitmapShader = AssetDatabase.LoadAssetAtPath<Shader>(TmpBitmapShaderPath);
            if (bitmapShader == null)
            {
                throw new GameFrameworkException(Utility.Text.Format("Can not load TextMeshPro bitmap shader at '{0}'.", TmpBitmapShaderPath));
            }

            var material = LoadSubAsset<Material>(assetPath, materialName);
            if (material == null)
            {
                material = new Material(bitmapShader)
                {
                    name = materialName
                };
        ...
        private static Shader LoadLegacyFontShader()
        {
            Shader shader = AssetDatabase.GetBuiltinExtraResource<Shader>("UI/Default Font.shader");
            if (shader == null)
            {
                throw new GameFrameworkException("Can not load built-in shader 'UI/Default Font.shader'.");
            }
