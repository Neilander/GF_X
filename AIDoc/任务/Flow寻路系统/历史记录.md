...
- `logic=81`：Invocation/Body 均约 `0.011ms`。因此原先 `StableGoal` 的约 `1.42ms` 未归属不是业务叶项，也不是 profiler 写入；它只在首轮调用出现，已锁定为调用边界冷启动（JIT/静态初始化尚未进一步区分），未做猜测性优化。
...
- `InvocationMax=3.625ms`、`BodyMax=2.044ms`；重复入口仍有约 `1.5ms` 总差额，参数访问、诊断记录、方法体和收尾均无法解释。当前仅剩调用边界/运行时冷启动未归属，禁止猜测性优化。
